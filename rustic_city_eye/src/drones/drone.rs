...
use super::{
    drone_config::DroneConfig, drone_error::DroneError, drone_location::DroneLocation,
...
pub struct Drone {
    ...
    send_to_monitoring_app: Sender<DroneLocation>,
}
...
impl Drone {
    ...
    fn update_location(&mut self) -> Result<(), DroneError> {
        ...
        if let Err(e) = self.send_to_monitoring_app.send(l) {
            return Err(DroneError::ProtocolError(e.to_string()));
        }
        Ok(())
    }
