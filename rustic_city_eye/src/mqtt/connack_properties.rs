...
impl ConnackProperties {
    pub fn write_to(&self, stream: &mut dyn Write) -> Result<(), ProtocolError> {
        ...
        let _ = writer.write_all(&[session_expiry_interval_id]).map_err(|_| ProtocolError::WriteError);
        ...
        let _ = writer.write_all(&[assigned_client_identifier_id]).map_err(|_| ProtocolError::WriteError);
        ...
        let _ = writer.write_all(&[authentication_method_id]).map_err(|_| ProtocolError::WriteError);
        ...
        let _ = writer.write_all(&[authentication_data_id]).map_err(|_| ProtocolError::WriteError);
        ...
        let _ = writer.write_all(&[response_information_id]).map_err(|_| ProtocolError::WriteError);
        ...
        let _ = writer.write_all(&[server_reference_id]).map_err(|_: Error| ProtocolError::WriteError);
        write_string(&mut writer, &self.server_reference)?;

        let reason_string_id: u8 = 0x1F_u8; //31
        print!("reason_string_id: {:?}", reason_string_id);
        let _ = writer.write_all(&[reason_string_id]).map_err(|_| ProtocolError::WriteError);
        ...
        let _ = writer.write_all(&[receive_maximum_id]).map_err(|_| ProtocolError::WriteError);
        ...
        let _ = writer.write_all(&[user_properties_id]).map_err(|_| ProtocolError::WriteError);
        ...
        let _ = writer.write_all(&[maximum_packet_size_id]).map_err(|_| ProtocolError::WriteError);
        ...
        let _ = writer.write_all(&[wildcard_subscription_available_id]).map_err(|_| ProtocolError::WriteError);
        ...
        let _ = writer.write_all(&[subscription_identifier_available_id]).map_err(|_| ProtocolError::WriteError);
        ...
        let _ = writer.write_all(&[shared_subscription_available_id]).map_err(|_| ProtocolError::WriteError);
        ...
        let _ = writer.write_all(&[server_keep_alive_id]).map_err(|_| ProtocolError::WriteError);
