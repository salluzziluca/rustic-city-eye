...
fn handle_map(elements_container: &gtk::Box,  monitoring_app_ref: Rc<RefCell<MonitoringApp>>){
    ...
    let event_box_rc = Rc::new(RefCell::new(event_box));
    event_box_rc.borrow().connect_button_press_event({
    ...
    add_camera_btn.connect_clicked({
        ...
        let event_box_rc = Rc::clone(&event_box_rc);
        move |_| {
            on_add_camera_clicked(Rc::clone(&y_ref), Rc::clone(&x_ref), &Rc::clone(&monitoring_app_ref), Rc::clone(&camera_list_box), Rc::clone(&event_box_rc));
        }
    });
    ...
    add_incident_btn.connect_clicked({
        ...
        let event_box_rc = Rc::clone(&event_box_rc);
        move |_| {
            on_add_incident_clicked(Rc::clone(&y_ref), Rc::clone(&x_ref), &Rc::clone(&monitoring_app_ref), Rc::clone(&incident_list_box), Rc::clone(&event_box_rc));
        }
    });
    ...
    update_camera_list(&camera_list_box.borrow(), &monitoring_app_ref, event_box_rc);

}

fn update_camera_list(camera_list_box: &gtk::Box, monitoring_app_ref: &Rc<RefCell<MonitoringApp>>,event_box: Rc<RefCell<gtk::EventBox>>) {
    ...
    let mut event_box_ref_mut = event_box.borrow_mut();
    let event_box = &mut *event_box_ref_mut;
    ...
    for camera in cameras {
       ...
       // Add camera image to the map at the camera location
       let camera_image = gtk::Image::from_file("src/monitoring/Camera.png");
       camera_image.set_margin_start(location.longitude as i32);
       camera_image.set_margin_top(location.latitude as i32);
       event_box.add(&camera_image);
...
fn on_add_camera_clicked(
    ...
    event_box: Rc<RefCell<gtk::EventBox>>
) {
    ...
    update_camera_list(&camera_list_box.borrow(), monitoring_app_ref, event_box);
}
