use std::{collections::HashMap, env, fs::File, io::BufReader, path::{Path, PathBuf}, sync::{mpsc::{self, Sender}, Arc, Condvar, Mutex}, thread, time::Duration};

use broker::broker::Broker;
use camera_system::camera_system::CameraSystem;
use client::client::{Client, ClientTrait};
use protocol::{client_message::{self, ClientMessage}, messages_config::MessagesConfig, publish::{payload_types::PayloadTypes, publish_config::PublishConfig}};
...
fn test01_new_camera_system_vacio() {
    ...
    let certs = certs(&mut BufReader::new(
        &mut File::open("./certs/cert.pem").unwrap(),
    ))
    .collect::<Result<Vec<_>, _>>()
    .unwrap();

    let private_key = private_key(&mut BufReader::new(
        &mut File::open("./certs/private_key.pem").unwrap(),
    ))
    .unwrap()
    .unwrap();
    ...
    let mut broker = match Broker::empty_broker(args, server_config) {
        Ok(broker) => broker,
        Err(e) => {
...
//     let root_dir = PathBuf::from(env::current_dir().expect("Failed to get current directory"))
//     .join("..")
//     .canonicalize()
//     .expect("Failed to canonicalize path");

// // Change the working directory to the project root
// env::set_current_dir(&root_dir).expect("Failed to set current directory");
...
//     thread::spawn(move || {
//         let mut camera_system = CameraSystem::<Client>::with_real_client(addr.to_string()).unwrap();
...
//         let mut camera_system = CameraSystem::<Client>::with_real_client(addr.to_string()).unwrap();
//         let channel = camera_system.camera_system_client.clone().receiver_channel;
//         let location = Location::new(10.0 / 10000.0, 0.0);
//         let id = camera_system.add_camera(location).unwrap();
//         let location2 = Location::new(11.0 / 10000.0, 0.0);
//         let id2 = camera_system.add_camera(location2).unwrap();
...
//         "./camera_system/packets_config/publish_config_test.json",
...
//         let (can_start_second_thread_clone, second_thread_completed_clone, camera_id_shared_clone) = (
...
//             let path1 = "src/surveilling/cameras/".to_string() + &camera_id.unwrap().to_string();
