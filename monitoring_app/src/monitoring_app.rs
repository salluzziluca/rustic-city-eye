...
use drones::drone_system::DroneSystem;
use protocol::client_message::{ClientMessage, Connect};
use protocol::disconnect::disconnect_config::DisconnectConfig;
...
use protocol::subscribe::subscribe_config::SubscribeConfig;
use protocol::subscribe::subscribe_properties::SubscribeProperties;
use utils::camera::Camera;
use utils::incident::Incident;
use utils::incident_payload::IncidentPayload;
use utils::location::Location;
use utils::protocol_error::ProtocolError;
use utils::single_disconnect_payload::SingleDisconnectPayload;
...
impl MonitoringApp {
    ...
    pub fn new(args: Vec<String>) -> Result<MonitoringApp, ProtocolError> {
        ...
        let drone_system =
            DroneSystem::new("packets_config/drone_config.json".to_string(), address.clone());
    ...
    pub fn add_drone_center(&mut self, location: Location) -> u32 {
        ...
        let _ = Persistence::add_center_to_file(
            center_id,
            location,
            self.drone_system.drone_config_path.to_string(),
            self.drone_system.address.to_string(),
        );
